...
void Freq_Multi_Bar () {
  ...
  if (flag.freq_multi_bar) {
    ...
    if (u.tune_multi == 1) {
      tft.fillRect(65, 88, 175, 2, ILI9341_BLACK ); //All first values changed from 100 and third from 140 to remove Mhz cursor on step change+++++++++
      tft.fillRect(65, 163, 175, 2, ILI9341_BLACK );
    ...
    if (u.tune_multi == 2) {
      tft.fillRect(65, 88, 175, 2, ILI9341_BLACK );
      tft.fillRect(65, 163, 175, 2, ILI9341_BLACK );
    ...
    //++++++++++++++++++++++Added to allow cursor under mHZ +++++++++++++++++++++
    if (u.tune_multi == 7) {
      tft.fillRect(65, 88, 175, 2, ILI9341_BLACK );
      tft.fillRect(65, 163, 175, 2, ILI9341_BLACK );

      if (flag.vfo_A_active == true) {
        tft.fillRect(67, 88, 14, 2, ILI9341_CYAN ); //experimenting with first value
      }
      if (flag.vfo_B_active == true) {
        tft.fillRect(67, 163, 14, 2, ILI9341_CYAN ); // experimenting with first value
      }
    }   //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  }
}
...
void display_update_vfo() {
  ...
  if (((flag.vfo_A_active == true) && ((flag.freq_Changed == true) || (flag.bandChanged == true)))  || (flag.vfo_A_B_changed == true))
  {
    ...
    //Ascertain if freq is less than 10Mhz and if so to clear the x10 screen area ++++++++++++++++++++++++++++++++++++++++++
    if (freqlength < 8) {
      int F10m_OffSet = -16;
  ...
  if ((((flag.vfo_B_active == true) && ((flag.freq_Changed == true) || (flag.bandChanged == true))) || (flag.ReDraw_Screen == true)) || (flag.vfo_A_B_changed == true))
  {
...
void display_update_rit() {
  ...
  u.RIT_A_value = u.RIT_Offset + u.vfo_A_value + RIT_A_val;//sets full RIT frequency for Si5351 ++++++++++++++++++++++++
  u.RIT_A_Display_value = u.RIT_Offset + RIT_A_val; //displays RIT as a +- deviation from main VFO ++++++++++++++++++++++++++
  u.RIT_B_value = u.RIT_Offset + u.vfo_B_value + RIT_B_val;//sets full RIT frequency for Si5351 ++++++++++++++++++++++++
  u.RIT_B_Display_value = u.RIT_Offset + RIT_B_val; //displays RIT as a +- deviation from main VFO +++++++++++++++++++++++++
...

  {
    ...
    //String freqtA=String(u.RIT_A_value);    // Text frquency convertrd - Amended to show RIT offset rather than complete frequency ++++++++++++++++++
    //tft.print(freqtA);
    tft.print(u.RIT_A_Display_value); //++++++++++++++ So as to display RIT as an offset without affecting Si5351 clock output
    tft.setCursor(RIT_A_hz_pos_x, RIT_A_freq_cursor_pos_y);// fix the Hz position to avoid over-writing as digits increase +++++++++++++++++++++++
    tft.print("   Hz"); 
    tft.setFont(DroidSansMono_10);
    ...
    //String freqtB=String(u.RIT_B_value);   // Text frquency convertrd - Amended to show RIT offset rather than complete frequency ++++++++++++++++++
    //tft.print(freqtB);
    tft.print(u.RIT_B_Display_value);
    tft.setCursor(RIT_B_hz_pos_x, RIT_B_freq_cursor_pos_y);// fix the Hz position to avoid over-writing as digits increase +++++++++++++++++++++++
    tft.print("  Hz"); 
